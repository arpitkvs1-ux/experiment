 ...
 */
(function (global) {
  var DOWNLOADED_POPUP_MSG = "Downloaded \uD83D\uDE0A";

  function notifyDownloaded() {
    try {
      if (typeof global.KV_showOkDialog === "function") {
        global.KV_showOkDialog(DOWNLOADED_POPUP_MSG);
        return;
      }
    } catch (e1) {
      /* ignore */
    ...
    try {
      global.alert(DOWNLOADED_POPUP_MSG);
    } catch (e2) {
      /* ignore */
    }
  }
  ...
  var VAAYU_ATTRIBUTION = "Generated by Vaayu App";

  function drawPdfAttributionTopRight(doc, pageW, marginMm, yMm) {
    doc.setFont("helvetica", "italic");
    doc.setFontSize(6.5);
    doc.setTextColor(120, 120, 120);
    doc.text(VAAYU_ATTRIBUTION, pageW - marginMm, yMm, { align: "right" });
  ...
  function buildAoA(title, data) {
    ...
    for (var i = 0; i < ncol; i++) {
      genRow.push(i === ncol - 1 ? VAAYU_ATTRIBUTION : "");
      schoolRow.push(i === 0 ? getSchoolName() : "");
      subRow.push(i === 0 ? getClassSubtitlePrefix() + title : "");
      blank.push("");
    }
    var aoa = [genRow, schoolRow, subRow, blank].concat(rows);
  ...
  function downloadExcel(title, data) {
    ...
    ws["!merges"] = [
      ...
      { s: { r: 2, c: 0 }, e: { r: 2, c: packed.ncol - 1 } },
    ];
    var wch = [];
  ...
  /** Multi-sheet workbook (e.g. consolidated mark slips). */
  function downloadWorkbookXlsx(wb, filename) {
    if (typeof XLSX === "undefined") {
      alert("Excel library not found. Ensure vendor/xlsx.full.min.js is present, then refresh.");
      return;
    }
    ...
    var fname = (filename || "KV_Workbook.xlsx").replace(/[\\/:*?"<>|]+/g, "_");
    if (typeof AndroidExport !== "undefined" && AndroidExport.saveFile) {
      try {
        var wbout = XLSX.write(wb, { bookType: "xlsx", type: "array" });
        AndroidExport.saveFile(
  ...
  function downloadPdf(title, data) {
    ...
    var margin = 14;
    drawPdfAttributionTopRight(doc, pageW, margin, 10);
    doc.setFont("helvetica", "bold");
    doc.setFontSize(14);
    doc.setTextColor(0, 51, 102);
    var schoolY = margin + 4;
    doc.text(getSchoolName(), pageW / 2, schoolY, { align: "center" });
    doc.setFont("helvetica", "normal");
    doc.setFontSize(12);
    doc.setTextColor(SUBTITLE_RGB[0], SUBTITLE_RGB[1], SUBTITLE_RGB[2]);
    doc.text(getClassSubtitlePrefix() + title, pageW / 2, schoolY + 7, { align: "center" });
    doc.setTextColor(0, 0, 0);
    var startY = schoolY + 14;
  ...
  function downloadMarksSlipPdf(meta, rows) {
    ...
    drawPdfAttributionTopRight(doc, pageW, margin, margin + 2);
    doc.setFont("helvetica", "bold");
  ...
  global.KVReports = {
    ...
    downloadWorkbookXlsx: downloadWorkbookXlsx,
