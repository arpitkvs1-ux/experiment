...
class SamagamFlowController(
    ...
    private fun injectAttendance(p: JSONObject) {
        ...
        webView?.evaluateJavascript(script) { raw ->
            phase = Phase.DONE
            val parsed = parseInjectResult(raw)
            setStatus(parsed.statusLine)
            notifyJsSamagamSummary(parsed.message, parsed.hasMismatch)
        }
    }
    ...
    private fun parseInjectResult(raw: String?): InjectResult {
        if (raw.isNullOrBlank() || raw == "null") {
            return InjectResult(
                message = "Present: 0\nAbsent: 0",
                hasMismatch = false,
                statusLine = "Attendance page loaded. Verify and press Submit on SAMAGAM."
            )
        }
        val cleaned = raw.trim().removeSurrounding("\"").replace("\\\"", "\"").replace("\\\\", "\\")
        return try {
            val obj = JSONObject(cleaned)
            val message = obj.optString("message", "Verify attendance on SAMAGAM and press Submit manually.")
            val hasMismatch = obj.optBoolean("hasMismatch", false)
            val statusLine = if (hasMismatch) {
                "Name mismatch — review the summary, then press Submit on SAMAGAM."
            } else {
                "Attendance synced. Press Submit on SAMAGAM."
            }
            InjectResult(message = message, hasMismatch = hasMismatch, statusLine = statusLine)
        } catch (_e: Exception) {
            InjectResult(
                message = "Filled attendance where possible. Verify and press Submit on SAMAGAM manually.",
                hasMismatch = true,
                statusLine = "Verify attendance on SAMAGAM."
            )
        }
    }

    private fun notifyJsSamagamSummary(message: String, hasMismatch: Boolean) {
        val payload = JSONObject()
            .put("samagamSummary", true)
            .put("message", message)
            .put("hasMismatch", hasMismatch)
        (activity as? MainActivity)?.notifySamagamFlowMessage(payload.toString())
    }

    private fun parseInjectMessage(raw: String?): String {
        return parseInjectResult(raw).message
    }
